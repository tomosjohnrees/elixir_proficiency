...
import debuggingAndTooling from "@/data/courses/elixir/topics/debugging-and-tooling";
import idiomaticElixir from "@/data/courses/elixir/topics/idiomatic-elixir";
import ets from "@/data/courses/elixir/topics/ets";
import typespecsAndDialyzer from "@/data/courses/elixir/topics/typespecs-and-dialyzer";
import guardsInDepth from "@/data/courses/elixir/topics/guards-in-depth";
...
describe("topicRegistry", () => {
  ...
  it("has 30 active topics", () => {
    ...
    expect(active[26].slug).toBe("ets");
    expect(active[27].slug).toBe("typespecs-and-dialyzer");
    expect(active[28].slug).toBe("debugging-and-tooling");
    expect(active[29].slug).toBe("guards-in-depth");
  });
...
describe("ets topic content", () => {
  it("has correct meta", () => {
    expect(ets.meta.slug).toBe("ets");
    expect(ets.meta.number).toBe(27);
    expect(ets.meta.active).toBe(true);
  });

  it("has ELI5 content with items and takeaways", () => {
    expect(ets.eli5.analogyTitle).toBeTruthy();
    expect(ets.eli5.analogy).toBeTruthy();
    expect(ets.eli5.items.length).toBeGreaterThan(0);
    expect(ets.eli5.keyTakeaways.length).toBeGreaterThan(0);
  });

  it("has visual content with data types", () => {
    expect(ets.visuals.dataTypes.length).toBeGreaterThan(0);

    ets.visuals.dataTypes.forEach((dt) => {
      expect(dt.name).toBeTruthy();
      expect(dt.color).toMatch(/^#/);
      expect(dt.examples.length).toBeGreaterThan(0);
    });
  ...
  it("has deep dive sections with prose", () => {
    expect(ets.deepDive.sections.length).toBeGreaterThanOrEqual(4);
    ets.deepDive.sections.forEach((section) => {
      expect(section.title).toBeTruthy();
      expect(section.prose.length).toBeGreaterThan(0);
    });
  });
  ...
  it("has at least 15 quiz questions", () => {
    expect(ets.quiz.questions.length).toBeGreaterThanOrEqual(15);
    ets.quiz.questions.forEach((q) => {
      expect(q.question).toBeTruthy();
      expect(q.options.length).toBeGreaterThanOrEqual(2);
      expect(q.explanation).toBeTruthy();
      const correct = q.options.filter((o) => o.correct);
  ...
  it("has 3 practice problems", () => {
    expect(ets.practice.problems).toHaveLength(3);
    ets.practice.problems.forEach((p) => {
      expect(p.title).toBeTruthy();
      expect(p.prompt).toBeTruthy();
      expect(p.hints.length).toBeGreaterThan(0);
      expect(p.solution).toBeTruthy();
...
describe("typespecsAndDialyzer topic content", () => {
  it("has correct meta", () => {
    expect(typespecsAndDialyzer.meta.slug).toBe("typespecs-and-dialyzer");
    expect(typespecsAndDialyzer.meta.number).toBe(28);
    expect(typespecsAndDialyzer.meta.active).toBe(true);
  });

  it("has ELI5 content with items and takeaways", () => {
    expect(typespecsAndDialyzer.eli5.analogyTitle).toBeTruthy();
    expect(typespecsAndDialyzer.eli5.analogy).toBeTruthy();
    expect(typespecsAndDialyzer.eli5.items.length).toBeGreaterThan(0);
    expect(typespecsAndDialyzer.eli5.keyTakeaways.length).toBeGreaterThan(0);
  });

  it("has visual content with data types", () => {
    expect(typespecsAndDialyzer.visuals.dataTypes.length).toBeGreaterThan(0);

    typespecsAndDialyzer.visuals.dataTypes.forEach((dt) => {
      expect(dt.name).toBeTruthy();
      expect(dt.color).toMatch(/^#/);
      expect(dt.examples.length).toBeGreaterThan(0);
    });
  ...
  it("has deep dive sections with prose", () => {
    expect(typespecsAndDialyzer.deepDive.sections.length).toBeGreaterThanOrEqual(4);
    typespecsAndDialyzer.deepDive.sections.forEach((section) => {
      expect(section.title).toBeTruthy();
      expect(section.prose.length).toBeGreaterThan(0);
    });
  });
  ...
  it("has at least 15 quiz questions", () => {
    expect(typespecsAndDialyzer.quiz.questions.length).toBeGreaterThanOrEqual(15);
    typespecsAndDialyzer.quiz.questions.forEach((q) => {
      expect(q.question).toBeTruthy();
      expect(q.options.length).toBeGreaterThanOrEqual(2);
      expect(q.explanation).toBeTruthy();
      const correct = q.options.filter((o) => o.correct);
  ...
  it("has 3 practice problems", () => {
    expect(typespecsAndDialyzer.practice.problems).toHaveLength(3);
    typespecsAndDialyzer.practice.problems.forEach((p) => {
      expect(p.title).toBeTruthy();
      expect(p.prompt).toBeTruthy();
      expect(p.hints.length).toBeGreaterThan(0);
      expect(p.solution).toBeTruthy();
...
describe("guardsInDepth topic content", () => {
  it("has correct meta", () => {
    expect(guardsInDepth.meta.slug).toBe("guards-in-depth");
    expect(guardsInDepth.meta.number).toBe(30);
    expect(guardsInDepth.meta.active).toBe(true);
  });

  it("has ELI5 content with items and takeaways", () => {
    expect(guardsInDepth.eli5.analogyTitle).toBeTruthy();
    expect(guardsInDepth.eli5.analogy).toBeTruthy();
    expect(guardsInDepth.eli5.items.length).toBeGreaterThan(0);
    expect(guardsInDepth.eli5.keyTakeaways.length).toBeGreaterThan(0);
  });

  it("has visual content with data types", () => {
    expect(guardsInDepth.visuals.dataTypes.length).toBeGreaterThan(0);

    guardsInDepth.visuals.dataTypes.forEach((dt) => {
      expect(dt.name).toBeTruthy();
      expect(dt.color).toMatch(/^#/);
      expect(dt.examples.length).toBeGreaterThan(0);
    });
  ...
  it("has deep dive sections with prose", () => {
    expect(guardsInDepth.deepDive.sections.length).toBeGreaterThanOrEqual(4);
    guardsInDepth.deepDive.sections.forEach((section) => {
      expect(section.title).toBeTruthy();
      expect(section.prose.length).toBeGreaterThan(0);
    });
  });
  ...
  it("has at least 15 quiz questions", () => {
    expect(guardsInDepth.quiz.questions.length).toBeGreaterThanOrEqual(15);
    guardsInDepth.quiz.questions.forEach((q) => {
      expect(q.question).toBeTruthy();
      expect(q.options.length).toBeGreaterThanOrEqual(2);
      expect(q.explanation).toBeTruthy();
      const correct = q.options.filter((o) => o.correct);
  ...
  it("has 3 practice problems", () => {
    expect(guardsInDepth.practice.problems).toHaveLength(3);
    guardsInDepth.practice.problems.forEach((p) => {
      expect(p.title).toBeTruthy();
      expect(p.prompt).toBeTruthy();
      expect(p.hints.length).toBeGreaterThan(0);
      expect(p.solution).toBeTruthy();
